                    ...
                    if (!isChoice) {
                      ...
                      return (
                        <label key={group.id} className={cx('flex cursor-pointer flex-wrap items-center gap-x-2 gap-y-1 rounded-lg border px-3 py-2 text-xs transition-colors', status === 'success' && 'border-good/50 bg-good/8', status === 'failure' && 'border-bad/50 bg-bad/8', status === 'pending' && 'border-accent/45 bg-accent/8', !status && (enabled[group.id] !== false ? 'border-line bg-panel hover:border-line-strong' : 'border-line/60 bg-canvas-soft opacity-55'))}>
                          ...
                          {isHidden && <span className="inline-flex items-center gap-1 rounded-full border border-warn/40 bg-warn/10 px-2 py-0.5 text-[10px] font-extrabold text-warn"><Icon name="eye-off" size={12}/>Hidden</span>}{group.options.some(({ index }) => existingDownloads[`${group.result.key}\0${index}`]) && <span className="inline-flex items-center gap-1 rounded-full border border-accent/45 bg-accent/12 px-2 py-0.5 text-[10px] font-extrabold text-accent-bright" title="Already in qBittorrent, so it will be skipped"><Icon name="download" size={12}/>Already in qBit · skipped</span>}
