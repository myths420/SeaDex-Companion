...
export default function ConfigTab({ config, status, username, onRunScan, onAccountUpdated, onSaved, onScannedDataCleared }: Props) {
  ...
  <section className="rounded-2xl border border-line bg-panel p-5"><header className="mb-4 flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-canvas-soft text-accent-bright"><Icon name="logs" size={19}/></span><div><h2 className="m-0 text-base font-extrabold">Diagnostics API</h2><p className="mt-1 mb-0 text-xs text-muted">Read-only log/status access by API key, separate from your login password</p></div></header><div className="space-y-3"><Field name="diagnostics_api_key" type="password" label="Diagnostics API key" hint="Send as the X-Diagnostics-Key header or a ?key= query parameter" placeholder="Generate a key below" configured={!!form.diagnostics_api_key_configured} onClear={() => setPendingClear('diagnostics_api_key')} form={form} set={set}/><button type="button" className={cx(buttonBase, 'border-line bg-canvas-soft text-muted')} onClick={() => set('diagnostics_api_key', crypto.randomUUID().replace(/-/g, ''))}><Icon name="refresh" size={14}/>Generate new key</button><div className="rounded-xl border border-line bg-canvas-soft p-3 text-[11px] text-muted"><p className="m-0"><strong className="text-ink">Logs:</strong> {location.origin}/api/diagnostics/logs</p><p className="mt-1 mb-0"><strong className="text-ink">Status:</strong> {location.origin}/api/diagnostics/status</p><p className="mt-2 mb-0">Disabled until a key is set here. Save configuration after generating or changing it.</p></div></div></section>
